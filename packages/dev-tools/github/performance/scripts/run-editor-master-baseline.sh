...
OUT_DIR="${PERF_RESULTS_DIR:-$PERF_RESULTS_DEFAULT}"
...
write_wp_env_for_master_plugin() {
	local plugin_path="$1"
	local mu_plugins_path="${ROOT_DIR}/${PERF_MU_PLUGINS_DIR}"
...
if [[ -f "${MASTER_PLUGIN_DIR}/.gitmodules" ]]; then
	...
	bash "${ROOT_DIR}/.github/actions/ensure-global-packages/ensure.sh" "${MASTER_PLUGIN_DIR}"
fi
...
bash "${ROOT_DIR}/packages/global-packages/packages/dev-tools/github/scripts/retry-wp-env-start.sh"
wait_for_wp
