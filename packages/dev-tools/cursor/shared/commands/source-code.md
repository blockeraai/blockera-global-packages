...
Each project root has a gitignored `source-codes/` symlink (created by `npm run project:bootstrap`) pointing at a shared folder from `BLOCKERA_EXTERNAL_SOURCE_CODES_PATH` in `.env`. Most trees are local clones of upstream GitHub repos; Blocksy trees are **distributed/build copies** (see below).
...
1. Paths are always relative to the **current project root** (`source-codes/...`).
2. Never use absolute user paths (`/Users/...`, `/home/...`).
3. Never substitute installed-site paths for clone paths (e.g. omit WordPress `src/`, or treat WooCommerce as only `plugins/woocommerce` without checking `packages/`).
4. If `source-codes/` is missing, run `npm run project:bootstrap` (requires `BLOCKERA_EXTERNAL_SOURCE_CODES_PATH` in `.env`).
