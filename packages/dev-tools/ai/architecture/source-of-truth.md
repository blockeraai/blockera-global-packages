...
Consumers receive the same tree via git submodule `packages/global-packages` (sparse-checkout `/packages/` only). Update the pin with the product script `npm run submodule:bump` or CI `sync-global-packages-submodule`. Do **not** copy packages with `blockera-folder-sync.json` (removed; older READMEs that mentioned it are stale).
